...
class UrlManager extends CUrlManager
{
     ...
     */
    public $hostInfo = 'http://karima';
    /**
     * @var string the host info used in SSL mode
     */
    public $secureHostInfo = 'https://karima';
    /**
    ...
    {
    	if(isset($params['isSecured'])){
    		$isSecured = $params['isSecured'];
    	} else {
    		$isSecured = true;
    	}
    	unset($params['isSecured']);
        ...
        $isSecuredConnection = Yii::app()->request->isSecureConnection;
        if ($isSecuredConnection) {
        	if($secureRoute && $isSecured){
            	$returnUrl = $this->secureHostInfo . $url;
        	} else {
        		$returnUrl = $this->hostInfo . $url;
        	}
        } else {
        	if($secureRoute && $isSecured){
            	$returnUrl = $this->secureHostInfo . $url;
     ...
     */
    protected function isSecureRoute($route,$isSecured)
    {
