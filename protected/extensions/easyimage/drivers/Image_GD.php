...
class Image_GD extends Image
{
	 ...
	 * @return  void
	 */
	protected function _do_resize($width, $height, $reqWidth, $reqHeight, $bgColor)
	{
		...
		if ($width > ($this->width / 2) AND $height > ($this->height / 2)) {
			...
			$image = $this->_create($pre_width, $pre_height,$bgColor);
		...
		// Create the temporary image to copy to
		$image = $this->_create($reqWidth, $reqWidth, $bgColor);
		$dst_x_pad = $dst_y_pad = 0;
		
		if(($reqWidth != $width) && ($reqWidth > $width)){
			$widthDiff = $reqWidth - $width;
			$dst_x_pad = max(round($widthDiff/2), 1);
		}
		
		if(($reqHeight != $height) && ($reqHeight > $height)){
			$heightDiff = $reqHeight - $height;
			$dst_y_pad = max(round($heightDiff/2), 1);
		}

		// Execute the resize
		if (imagecopyresampled($image, $this->_image, $dst_x_pad, $dst_y_pad, 0, 0, $width, $height, $pre_width, $pre_height)) {
			// Swap the new image for the old one
			imagedestroy($this->_image);
			$this->_image = $image;
	 ...
	 * @return  resource
	 */
	protected function _create($width, $height, $bgColor)
	{
