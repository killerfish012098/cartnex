...
class EasyImage extends CApplicationComponent
{
    ...
    /**
     * @var string value of color: #FFFFFF; for image background
     */
            ...
            switch ($key) {
                case 'resize':
                    $this->resize(
                        ...
                        isset($value['bgColor']) ? $value['bgColor'] : 'FFFFFF'
                    );
                    break;
                case 'crop':
    ...
    public function thumbSrcOf($file, $params = array())
    {
        ...
        */
    	$webUrlPath = Yii::app()->params['config']['site_url'].'cache/';
		$cachePath = Yii::getpathOfAlias('webroot').DIRECTORY_SEPARATOR.'cache';
		if(isset($file) && ($file != "") && file_exists($file)){
			$file = $file;
		} else {
			$file = Library::getMiscUploadPath().Yii::app()->config->getData('CONFIG_STORE_NO_IMAGE');
		}
		$fileName = pathinfo($file, PATHINFO_FILENAME);
		$fileExt = isset($params['type']) ? $params['type'] : pathinfo($file, PATHINFO_EXTENSION);
		$cacheFileName = $fileName.'-'.$params['resize']['bgColor'].'-'.$params['resize']['width'].'x'.$params['resize']['height'].'.'.$fileExt;
		$cacheFile = $cachePath.DIRECTORY_SEPARATOR.$cacheFileName;
		$webCacheFile = $webUrlPath.$cacheFileName;
    ...
    public function resize($width = NULL, $height = NULL, $master = NULL, $bgColor)
    {
        return $this->image()->resize($width, $height, $master, $bgColor);
    }
